...
def _value_history(symbol: str) -> pd.DataFrame | None:
    ...
    try:
        ...
        df = df[["date", "PE(TTM)", "市净率", "市销率"]].rename(columns={
            "PE(TTM)": "pe_ttm", "市净率": "pb", "市销率": "ps",
        })
...
def _pick_at(asof: pd.Timestamp, universe: list[dict],
             ...
             ind_map: dict | None = None) -> pd.DataFrame:
...
def run_backtest(
    ...
    # 结果级缓存（6 小时）
    cache_key = (f"result_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}"
    ...
    for i, m_start in enumerate(months):
        # 选股
        picks = _pick_at(m_start, universe, top_n=top_n,
